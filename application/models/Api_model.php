...
public function getUserObject($id) {
        $query = $this->db
                ->select("id as user_id,active,username,name,email,phone,CONCAT('".base_url()."uploads/',image) AS profilePic,referal_code,lang as language")
                ->where('id', $id)
                ->limit(1)
                ->get('users');
				$data = array();
...
//	$this->db->where(array('user_id'=>$user_id))->delete($this->tbl_session);
    ...
    public function getUserById($id) {
       /* $user = $this->db
                ->select("id as user_id,name,phone,address,bio,device_id,devicetype,latitude,longitude,CONCAT('".base_url()."uploads/', image) as image")
                ->where('id', $id)
                ->limit(1)
                ->get('users')
                ->result_array();*/
				$user = $this->db
                ->select("u.id as user_id,u.name,u.phone,u.address,u.bio,u.device_id,u.devicetype,u.latitude,u.longitude,CONCAT('".base_url()."uploads/', u.image) as image,u.class_id,c.class_name")
				->join('classes as c','c.id=u.class_id')
                ->where('u.id', USER_ID)
                ->limit(1)
                ->get('users as u')
                ->result_array();
                                             ...
                                             <tr>
                                                <td style="background-color:white;padding-top:30px;padding-bottom:30px">
                                                   <table border="0" cellpadding="0" cellspacing="0" width="100%">
                                                      <tbody>
                                                         <tr>
                                                            <td align="center" style="padding-top:0;padding-bottom:20px"> <a > <img src="'.base_url().'assets/logo.png" alt="" width="120" height="50" style="vertical-align:middle" class="CToWUd"> </a> </td>
                                                         </tr>
                                                         <tr>
                                                         ...
                                                         <tr>
                                                            <td   style="font-family:Helvetica,Arial,sans-serif!important;font-size:16px;line-height:24px;word-break:break-word;padding-left:20px;padding-right:20px;padding-top:20px"> Please verify your email address so you can get full access to our plateform. </td>
                                                         </tr>
                                                         <tr>
