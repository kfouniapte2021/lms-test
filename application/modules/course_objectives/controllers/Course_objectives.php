<?php
 ...
 class Course_objectives extends MX_Controller {
 ...
 $aData['course_id'] =$course_id;
	...
	function save(){ 
		...
		$this->form_validation->set_rules('selection_type', 'Selection type', 'trim|required');
		if ($this->form_validation->run()==false){
			$arr = array("status"=>"validation_error" ,"message"=> validation_errors());
			echo json_encode($arr);
		}
		else{
$course_objectivesData=array(
		'title'=>$title,
		...
		$iftrueArray = $_POST['iftrue'];
		$result = $this->crud->saveRecord($PrimaryID,$course_objectivesData,$this->tbl);
		if(empty($PrimaryID)){
	...
	$index=0;
						foreach ($option_list as $key => $value){
							$aincludes[] = array('question_id'=> $insrtID,'answer'=>$value,'is_true'=>$iftrueArray[$index]);
$index++;
						}
					if($this->db->insert_batch($this->tbl_course_objectives_options,$aincludes) >0)
					{
						$result =1;
					}
				  }
				 ...
				 $this->db->where('question_id', $PrimaryID);
				if($this->db->delete($this->tbl_course_objectives_options)){
					if(!empty($title)){
