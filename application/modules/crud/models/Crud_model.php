...

{
    ...
    /*||||||||Courses crasula module||||||||*/
function getActiveSessionStaff(){
	
return 	$this->db->select('id,name,email,image,phone')->where(array('user_type'=>STAFF))->get(TBL_USER);
	}
	
function getActiveSessionClasses(){
	$data = array();
	$query =$this->db->where('status',1)->get('batch_session');
if($query !== FALSE && $query->num_rows() > 0){
$batch_id=	$query->row()->id; // active batch id
//$batch_id=	3;
   $query=$this->db->select('id,class_name')->where(array('batch_id'=>$batch_id))->get('classes');
...
WHERE status = " . $this->active . " AND TS.user_id = '" . $userid . "'");
  ...
  public function customiseorderdetail($id)
    ...
    'smtp_port' => 465,
    'smtp_user' => 'noreply@cyphersol.com',
    'smtp_pass' => 'kFZ@iX1H}dAh',
    'mailtype'  => 'html',
    'charset'   => 'utf-8'
        ...
        $this->email->from('noreply@zukulee.com', $from_heading);
...

    {
        ...
        if (!empty($id)) {
            ...
            if (!empty($key)) {
                ...
                $ql = $this->db->select('id')->from($table)->where($key, $id)->get();
