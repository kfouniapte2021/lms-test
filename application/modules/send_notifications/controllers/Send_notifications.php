...
class Send_notifications extends MX_Controller {
	...
	public function index(){  
		...
		$aData['data'] =$this->db->query('SELECT u.id,u.name,u.email,u.image FROM `users`as u
WHERE u.user_type=2');
if(isset($_GET['class_id']) and $_GET['class_id']!=''){
	$aData['data'] =$this->db->query('SELECT u.id,u.name,u.email,u.image FROM `users`as u
WHERE u.user_type=2 and class_id='.$_GET['class_id']);
	}
	$aData['activeSessionClasses'] =$this->crud->getActiveSessionClasses();
