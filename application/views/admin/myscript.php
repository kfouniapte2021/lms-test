    ...
    contentType: false,
        success: function(response) {
            ...
            {   
					...
					setTimeout(function(){
						window.location="<?=base_url() ?>"+'auth/create_user';
						},2000);
		   }
...
-------------------------------------------------------
**/
function closedisputeresetorder(order_id,dispute_id){
		 $.confirm({
    title: 'Confirmation!',
    content: 'Are you sure to you want to reset this order for again processing.?!',
	animation: 'zoom',
	...
	buttons: {
        deleteUser: {
            ...
            action: function () {
				...
				//ajax call to delete	
				$.ajax({
				url: "<?php echo base_url().'Disputeconversation/deletedisputeconversation_and_reset_order'; ?>",
				type: 'POST',
				data: {orderid:order_id,disputeid:dispute_id},
				dataType: "json",
				success: function(response) {
					$(".ui-item").hide();
				if (response.status == 1)
				...
				showAlert(response.message,'Success','green');
				setTimeout(function(){
					window.location="<?=base_url() ?>"+'disputedorder';
				},2000);
				}
				else if (response.status ==0)
				{  
	...
	buttons: {
        deleteUser: {
            ...
            action: function () {
				...
				//ajax call to delete	
				$.ajax({
				url: "<?php echo base_url().'crud/userdelete'; ?>",
				type: 'POST',
