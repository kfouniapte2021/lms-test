...

{
...

	{
		...
		$this->identity_column2 = $this->config->item('identity2', 'ion_auth');
...
// just uncommnet  that part for avioid duplication
...
// just uncommnet  that part for avioid duplication
		...
		$aemail =  explode('@',$email); // arslan

	 	$aemail =  preg_replace("/[^A-Za-z0-9 ]/", '', $aemail);
		...
		$data = array(
		   ...
		   // $this->identity_column   => $identity,   
			  ...
			  $this->identity_column   => $aemail['0'].'-'.rand(), 

		    'username'   => $aemail['0'].'-'.rand(),
		...
		$dbExi = $this->db->insert($this->tables['users'], $user_data);
...
//echo $remember;exit;
	...
	$query  = $this->db->query("SELECT *  FROM `users`  WHERE (".$this->identity_column." = '".$identity."') 

	OR (".$this->identity_column2." ='".$identity."')  
...
$afftectedRows = $this->db->affected_rows();
...

	{
		...
		public function users_by_current_user()
...

	{
		...
		$sql="SELECT * FROM users where added_by=".$this->session->userdata("user_id").""; 
...

	{
		...
		$this->trigger_events('user_locations');
		...
		$this->order_by($this->tables['user_locations'].'.user_id', 'desc');



		$this->where($this->tables['user_locations'].'.user_id', $id);
