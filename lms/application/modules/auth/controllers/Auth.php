...
class Auth extends CI_Controller {
	...
	public function admins($user_ype=1)
	{
...

		{
			...
			//list the users
  $where  = array('user_type'=>$user_ype,'user_type'=>$user_ype);
  ...
  $where  = array('user_type'=>STAFF);
			$data =  $this->db->select('u.*,s.title as subject,cl.class_name')
			->join('subjects as s','s.id=u.subject_id','left')
			->join('classes as cl','cl.id=u.class_id','left')
			->where($where)->get(TBL_USER.' as u');
			...
			$this->_render_page('staff_view', $this->data);
			...
			{   
					if($this->ion_auth->email_check($email))
					{
						...
						if($this->db->update('users',array('password'=>$string)))
						{
							...
							$from='noreply@skillsqrd.com';
							//$from_heading='Skillsqard Recovery Password';
							$subject='Password Recovery Request From Zukulee.com ';
							if($this->input->ip_address()!='127.0.0.1')
							{
...
public function getsubject(){
	$this->crud->verifyRequiredParams(array("class_id"));
	$data =$this->db->where(array('class_id'=>$_POST['class_id']))->get('subjects');
	...
	if($data->num_rows()){
		...
		foreach($data->result() as $subject){
			$option.='<option value="'.$subject->id.'">'.$subject->title.'</option>';
			}
		...
		$option.='</select>';
		echo json_encode(array('status'=>200,'data'=>$option));
	}
...
$aData['classes'] =$this->db->get($this->tbl_classes);
	...
	/********************************************************************************************************************/
     public function createaccount()
		...
		$this->form_validation->set_rules('class_id','Choose Class','required');
        ...
        {
			$Transactiondata =$this->stripePayment();
			//pre($_POST);
			...
			if(isset($_POST['referred_by']) and $_POST['referred_by']!=''){
				if(ifvalidref($_POST['referred_by'])){
					$additional_data['referred_by']=$_POST['referred_by'];
					}else{
						$arr = array('status' => 0,'message' => "Refrel code is not valid");
		  ...
		  if($error=='')
		  {
			...
			else
			{
			 ...
			 switch($result){
				...
				case 1:
				$Transactiondata['user_id']=$this->db->insert_id();
			...
			/*	* send confirmation email **/
				...
				$to=$email;
				$from='noreply@Zukulee.com';
				$from_heading='Zukulee';
				...
				$arr = array('status' => 0,'message' => "Not Saved! ".dberror());
				...
				$arr = array('status' => 0,'message' => "Error Not Saved! ".dberror());
				...
				break;	
				  ...
				  $arr = array('status' => 0,'message' => ' '.$error .dberror());
		...
		$TransactionData['package_id']=0;
          if ($this->db->insert('order_card_detail', $TransactionData))
			...
			return 1;
                  ...
                  {
                    $data['status']  = 204;
            $data['message'] = 'transaction done but not saved in db'.dberror();
            echo json_encode($data);
            exit;
                  }
		}
            ...
            if (is_array($chargeJson) && $chargeJson['object'] == 'charge')
              {
                ...
                $buyerHtml .= '<p>Thanks for your Payment you can now browse all coursed videos</p>';
                
                $buyerHtml .= '<br><br><br><br><a href="' . base_url() . '" target="_blank">Skillsquared Team </a>';
                $subjectLineBuyer = 'Payment done on Zukulee LMS';
                                                ...
                                                <td style="background-color:white;padding-top:30px;padding-bottom:30px">
                                                   <table border="0" cellpadding="0" cellspacing="0" width="100%">
                                                      <tbody>
                                                         ...
                                                         <tr>
                                                            <td   style="font-family:Helvetica,Arial,sans-serif!important;font-size:16px;line-height:24px;word-break:break-word;padding-left:20px;padding-right:20px;padding-top:20px"> Welcome to Zukulee! </td>
                                                         </tr>
                                                         <tr>
                                                         ...
                                                         <tr>
                                                            <td   style="font-family:Helvetica,Arial,sans-serif!important;font-size:16px;line-height:24px;word-break:break-word;padding-left:20px;padding-right:20px;padding-top:30px">
                                                               <div style="padding-top:10px">Thanks for your time,<br>The Zukulee Team</div>
                                                            </td>
                                                         </tr>
                                                      </tbody>
                                                   </table>
...
if($user_type==USER){
			$this->form_validation->set_rules('class_id', 'Class / Grade', 'trim|required');
...
if(isset($designation)){
	$additional_data['designation']=$designation;
	}
...
if($user_type==ADMIN){
    ...
    padding: 30px;
    text-align: center;
    box-shadow: 1px 1px 8px 2px #ccc0c0;
    width: 45%;
    margin-left: 20%;
    border-radius: 20px;
    background-color: #a9a9a908;
">';
                $EmailBody .= 'Dear <b>'.ucfirst($name).'</b><br>';
                $EmailBody .= '<p>You can login on Zukulee LMS by using below details:</p>';
                $EmailBody .= '<p><b>Email:</b> '.$email.'</p>';
                $EmailBody .= '<p><b>Password:</b> '.$password.'</p>';
                
                $EmailBody .= '<br><br><br><br><a href="' . base_url() . '" target="_blank">Zukulee LMS Team </a>';
                $subjectLineBuyer = 'Login Credentials for  Zukulee LMS';
				if($this->input->ip_address()!='127.0.0.1')
				{
                	$this->crud->send_mail($email,FROM,HEADING,$subjectLineBuyer,$EmailBody);
				}/*********************************************************************************/
			 ...
			 switch($result){
				...
				$arr = array('status' => 1,'redirect' => $redirect,'message' => "Saved successfully !".$error);
       ...
       $is_unique =  '';
...

        {
			...
			/** send confirmation email **/
			 ...
			 $htmlContent.='<br><br><p><center> Zukulee SUPPPORT TEAM </center> </p>';
			 $to=$email;
			 $from='noreply@Zukulee.be';
			 $from_heading='Zukulee';
