...
/**===========Services save==============**/
	  $('#service_create').on("submit",function(e) {
	   ...
	   //  
		...
		/*	if (filesToUpload.length > 0) {
			...
			if($('#service_banner').val()!='')
			{
				formData.append("service_banner", document.getElementById('service_banner').files[0]);
			}
			if($('#service_banner2').val()!='')
			{
				formData.append("service_banner2", document.getElementById('service_banner2').files[0]);
			}
			
			if($('#service_banner3').val()!='')
			{
				formData.append("service_banner3", document.getElementById('service_banner3').files[0]);
			}
			if($('#service_banner4').val()!='')
			{
				formData.append("service_banner4", document.getElementById('service_banner4').files[0]);
			}
			if($('#service_banner5').val()!='')
			{
				formData.append("service_banner5", document.getElementById('service_banner5').files[0]);
			}
			
			
				
		
		var other_data = $('#service_create').serializeArray();
		...
		formData.append("description", post_description);
		   ...
		   $.ajax({
			type: "POST",
			url: "<?php echo base_url().'seller/service_create'; ?>",
			data: formData,
			cache: false,
			contentType: false,
			processData: false,
		...
		//	$('#form_add_update .btn_au').addClass('hidden');
            ...
            {   
				...
				setTimeout(function(){
				  window.location='<?php echo base_url().'seller/manageServicves'; ?>';
				},2000);
			}
  ...
  $("#price-ranges").on('click','input',function(){
		...
		else
		{
			...
			get_filter_service( catgeoresfiltervar,pricerange,locationsfiltervar,statusfiltervar,radius_range);
	 ...
	 $("#categories-filter").on('click','input',function(){
	   ...
	   if(locationsfilter!='')
	   {
		...
		var radius_range = $("#radius_range").val();
		get_filter_service(catgeoresfilter,pricefilter,locationsfiltervar,statusfiltervar,radius_range);
    });
	...
	// $("#locations-filter").on('change','#city_id',function(){
	 $("#locations-filter").on('change','select',function(){
		...
		var radius_range = $("#radius_range").val();
	   get_filter_service(catgeoresfiltervar,pricefilter,locationsfilter,statusfiltervar,radius_range);
    });
	...
	/* $("#locations-filter").on('click','input',function(){
	 ...
	 $("#status-filter").on('click','input',function(){
	   ...
	   var radius_range = $("#radius_range").val();
	   get_filter_service(catgeoresfiltervar,pricefilter,locationsfiltervar,statusfilter,radius_range);
    });
	  ...
	  $(".btn-byname-search").click(function(){
	   ...
	   if($("#textsearchbyname").val()!='')
	   {
			...
			var statusfilter   ='-1';
			var radius_range   ='-1';
			get_filter_service(catgeoresfilter,pricerange,locationsfilter,statusfilter,radius_range,textfieldbyname);
	   }
...
/************************** edit user ***********************************/
   function chooselocationlevel()
   {
	 ...
	 //return false;
      ...
      function get_filter_service(catgeoresfilter,pricerange,locationsfilter,statusfilter,radiusrange,textfbyname='')
      {
	   <?php 
	    $s= '';
	   ...
	   NProgress.start();
		jQuery.ajax({
			...
			data:{'pricerange':pricerange,'categoryfilter':catgeoresfilter,'locationsfilter':locationsfilter,'statusfilter':statusfilter,'radiusrange':radiusrange,'textfbyname':textfbyname}, 
...
return true;
   ...
   function removeimg(obj,rowid)
   {
	...
	buttons: {
		...
		action: function () {
		$.ajax({
				url: "<?php echo base_url().'crud/deleteimg'; ?>",
				type: 'POST',
				data: {id:key},
				dataType: "json",
				success: function(response) 
