...
function cleanURL($textURL) {
  $URL = strtolower(preg_replace( array('/[^a-z0-9\- ]/i', '/[ \-]+/'), array('', ' '), $textURL));
	 ...
	 function dberror(){
		 $CI =& get_instance(); 
		 $e = $CI->db->error(); // Gets the last error that has occured
$arr = array("status"=>204 ,"error_code"=>$e['code'] ,"message"=> $e['message']);
...
function current_batch_id()
		...
		$data =$CI->db->select('*')->from('batch_session')->where('status',1)->limit(1)->get();

		if ($data->num_rows()>0){

			return $data->row()->batch_id;
		}
		else{
			return 0;
		}
 ...
 function getClassTitle($class_id){
	 $CI =& get_instance(); 
		$data= 	$CI->db->select('*')->where(array('id'=>$class_id))->get('classes');
		
		if($data->num_rows()>0){
			return $data->row()->class_name;
			}
		}
 function ifvalidref($referal_code=''){
	 
	 $CI =& get_instance(); 
		$data= 	$CI->db->select('*')->where(array('referal_code'=>$referal_code))->get('users');
 ...
 function getClassBatch($class_id){
	 $CI =& get_instance(); 
		$data= 	$CI->db->select('b.batch_id')
		->join('batch_session as b','c.batch_id=b.id')
		->where(array('c.id'=>$class_id))
		->get('classes as c');
...
if ($data->num_rows()>0){
...

	{
	 ...
	 function getWhere($table,$arr){ 
		...
		$CI->db->select('*')->from($table)->where($arr);
...
function get_thumbnail($image)
{
	...
	$img = base_url() . 'uploads/noimg.png';
	if (!empty($image)) {
		$aImage = explode('.', $image);
		$file =   FCPATH . '/uploads/' . $aImage[0] . '_thumb' . '.' . $aImage[1];
		if (is_file($file)) {
			$img =   base_url() . 'uploads/' . $aImage[0] . '_thumb' . '.' . $aImage[1];
		} else {
			if (is_file(FCPATH . '/uploads/' . $image)) {
				$img = base_url() . '/uploads/' . $image;
			}
		}
	}
...
function get_bookcover($image)
{
	//base_url() . 'uploads/' . $image;
	$img = base_url() . 'uploads/default_cover.png';
	if (!empty($image)) {
		$aImage = explode('.', $image);
		$file =   FCPATH . $aImage[0]  . '.' . $aImage[1];
		if (is_file($file)) {
			$img =   base_url()  . $aImage[0]  . '.' . $aImage[1];
		} else {
