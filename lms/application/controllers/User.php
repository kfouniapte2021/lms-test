...
class User extends CI_Controller
{
	...
	function register()
    {
        $aData['classes'] = $this->db->select('id,class_name ')->from('classes')->order_by("id", "asc")->get();
        $this->load->view('register', $aData);
    }
    function login()
    {
    ...
    {
        $aData['page_title'] = 'Trainer  ';
        $trainer             = $this->db->where(array(
            'active' => 1,
            'user_type' => STAFF,
            'id' => $id
        ))->get(TBL_USER);
        if ($trainer->num_rows() > 0) {
            $aData['page_title'] = 'Trainer | ' . $trainer->row()->name;
        }
        $aData['trainer'] = $trainer;
        $this->load->view('single-instructor', $aData);
    }
    function testmail()
    {
    ...
    {
        $this->crud->verifyRequiredParams(array(
            "class_id"
        ));
        ...
        if ($data->num_rows()) {
            $option .= '<option>Select Subject</option>';
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (isset($_FILES['image']['name']) and !empty($_FILES['image']['name'])) {
                ...
                if (in_array($ext, array(
                    ...
                    $arr = array(
                        "status" => "validation_error",
                        "message" => 'Invaild Certificate fles type'
                    );
                    echo json_encode($arr);
                    exit;
                }
...
<a href="javascript:void(0);" onclick="deleteaddional(\'certificate\',\'' . base64_encode($data[$index]->id) . '\',\'' . $index . '\',\'cerlistid\')"><span class="glyphicon glyphicon-remove-circle"></span></a></td>';
                    ...
                    $arr = array(
                        'status' => 1,
                        'message' => "Certificate has been Saved Succefully !",
                        'htmlrow' => $html
                    );
                    echo json_encode($arr);
                    break;
...
<a href="javascript:void(0);" onclick="deleteaddional(\'certificate\',\'' . base64_encode($data[$index]->id) . '\',\'' . $index . '\',\'cerlistid\')"><span class="glyphicon glyphicon-remove-circle"></span></a>
                    ...
                    $arr = array(
                        'status' => 2,
                        'message' => "Certificate has been Updated Succefully !",
                        'htmlrow' => $html
                    );
                    echo json_encode($arr);
                    break;
...
<a href="javascript:void(0);" onclick="deleteaddional(\'education\',\'' . base64_encode($data[$index]->id) . '\',\'' . $index . '\',\'edulistid\')"><span class="glyphicon glyphicon-remove-circle"></span></a>
                    ...
                    $arr = array(
                        'status' => 1,
                        'message' => "Education has been Saved Succefully !",
                        'htmlrow' => $html
                    );
                    echo json_encode($arr);
                    break;
...
<a href="javascript:void(0);" onclick="deleteaddional(\'education\',\'' . base64_encode($data[$index]->id) . '\',\'' . $index . '\',\'edulistid\')"><span class="glyphicon glyphicon-remove-circle"></span></a></td>';
                    ...
                    $arr = array(
                        'status' => 2,
                        'message' => "Education has been Updated Succefully !",
                        'htmlrow' => $html
                    );
                    echo json_encode($arr);
                    break;
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (empty($identityfile)) {
                if (empty($_FILES['identityfile']['name'])) {
                    $arr = array(
                        'status' => 0,
                        'message' => 'The <i class="glyphicon glyphicon-arrow-right"></i> CNIC/Driving Licience is required!'
                    );
                    echo json_encode($arr);
                    die();
                }
            ...
            if (isset($_FILES['identityfile']['name']) and !empty($_FILES['identityfile']['name'])) {
                ...
                if (in_array($ext, array(
                    ...
                    $arr = array(
                        'status' => "validation_error",
                        'message' => "Invalid File Format for CNIC/ Licence"
                    );
                    echo json_encode($arr);
                    exit;
                }
            ...
            for ($index = 0; $index < count($freelancerskils); $index++) {
                if (!empty($freelancerskils[$index])) {
                    $afreelancerskills[] = array(
                        'freelancer_id' => $freelancer_id,
                        'skill_id ' => $freelancerskils[$index]
                    );
                }
            }
            ...
            switch ($result) {
                case 1:
                    $arr = array(
                        'status' => 1,
                        'message' => "Your Role has been saved Succefully !",
                        'data_user' => $username
                    );
                    echo json_encode($arr);
                    break;
                case 2:
                    $arr = array(
                        'status' => 2,
                        'message' => "Your Role has been Updated Succefully !",
                        'data_user' => $username
                    );
                    echo json_encode($arr);
                    break;
